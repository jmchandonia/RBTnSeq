...
RBTnSeqClient <- function(url, token = NULL, flatten = TRUE, 
    ...
    ret_client_object_ref[['runTnSeq']] <- function(input_params) {
        ret <- ret_client_object_ref[['json_rpc_call']]("RBTnSeq.runTnSeq", list(
                input_params
        ))
        return(ret[[1]])
    }
    ...
    class(ret_client_object_ref) <- 'RBTnSeqClient'
